...
const ConfirmAccountModal = (props) => {
    ...
    const sendCode = () => {
        form.handleSubmit(async (inputs) => {
            ...
            setSending(true);
            let result = await client.post('/accounts/confirm/send', inputs);
            setSending(false);
    ...
    useEffect(() => {
        ...
        form.setValue('username', inputs.username);
        sendCode();
    }, []);
        ...
        <>
            ...
            <Modal.Body as={Form} onSubmit={preventDefault(() => submit())}>
                <div className="text-center mb-5">
                    <h4>{!codeSent ? 'Confirm account' : 'Enter security code'}</h4>
                    ...
                    <div className="col-12 mt-5">
                        <button className="btn btn-primary  w-100" type="submit" disabled={(codeSent ? submitting : sending)}>
                            <div className="position-relative d-flex align-items-center justify-content-center">
                                <div className={`${(codeSent ? submitting : sending) ? 'invisible' : ''}`}>{codeSent ? 'Confirm account' : 'Send code'}</div>
