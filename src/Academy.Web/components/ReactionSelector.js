...
const icons = (() => {
    ...
    return {
        find: (scope, name) => {
            return variants[scope] && variants[scope][name] || 'n/a'
        },
    }
})();
    ...
    const getReactions = () => () => {
        const humanize = ((count, type) => {
            ...
            if (type == selectedType) {
                ...
                else comment += `You and ${count} people `;

                comment += `${(type == 'like' || type == 'love') ? `${type} this` : `are ${type}`}`;
            }
            else {
                if (count == 1) comment += `${count} person `;
                else comment += `${count} people `;
        ...
        return props.reactions.map(({ type, count }) => {
            return {
                type,
                count,
                comment: {
                    like: humanize(count, type),
                    love: humanize(count, type),
                    happy: humanize(count, type),
                    surprised: humanize(count, type),
                    sad: humanize(count, type),
                    angry: humanize(count, type),
                }[type]
            }
        })
    };
        ...
        <div style={{ display: "inline-block" }}>
            <div className="border rounded-pill" style={styles.selector}>
                {_.map(reactions, (reaction) => {
                    return (
                        <div style={styles.icon} key={reaction.type}>
